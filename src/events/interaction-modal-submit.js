...
module.exports = {
	...
	async execute(interaction) {
		...
		if (interaction.customId.startsWith("newpost_")) {
			...
			if (!additionalData) return await interaction.editReply({ content: "Es ist ein Fehler aufgetreten!\nBitte versuche es erneut.", ephemeral: true });
			...
			if (!channel.isTextBased()) return await interaction.editReply({ content: "Der angegebene Channel ist kein Textkanal!", ephemeral: true });
			...
			try {
				...
				await channel.send({ content: data.ping ? (data.ping.name == "@everyone" ? "@everyone" : `<@&${data.ping.id}>`) : "", embeds: [embed] });
			} catch (error) {
				logger.error(error);
